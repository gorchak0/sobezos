...
func (s *Service) UserAdd(TelegramID int, args string) (string, error) {
	...
	if len(parts) < 3 {
		...
		return "⚙️Используйте: /add <telegram_id> <username> <role>", errors.New("Используйте: /add <telegram_id> <username> <role>")
	}
	...
	if err != nil {
		...
		return "⚠️Ошибка формирования запроса", ErrServiceUnavailable
	}
	req, err := http.NewRequest("POST", url, bytes.NewReader(body))
	if err != nil {
		s.logger.Error("Ошибка создания запроса к user-service", zap.Error(err))
		return "⚠️Ошибка запроса к user\\-service", ErrServiceUnavailable
	}
	...
	return "⚠️Ошибка", ErrServiceUnavailable
}
